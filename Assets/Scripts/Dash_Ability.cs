...
public class Dash_Ability : Ability
{
    ...
    private Vector3 inclinedUpForward, inclinedDownForward;
    ...
    public override void Initialize(GameObject obj)
    {
        ...
        inclinedUpForward = new Vector3(-1.0f, 0.3f, 0.0f);
        inclinedDownForward = new Vector3(0.9f, -0.3f, 0.3f);
    ...
    public override void TriggerAbility()
    {
       ...
       //raycast both hit something
        if (playerOrientationHit && camRayHit)
        {
            ...
            {
                player.transform.position += inclinedUpForward * dashDistance;
            }
            else
            {
               ...
               Debug.Log(distance);
                if(distance > 10.7 && distance <= 13.8)
                {
                   // Debug.Log("a1");
        ...
        else if (!playerOrientationHit && camRayHit)
        {
           ...
           //mainly used for ariel dashing on this else if block
            if (pmScript.isGrounded())
                ...
                player.transform.position += pmScript.getPlayerCam().transform.forward * (dashDistance/5);
        }
        else if(!playerOrientationHit && !camRayHit)
        {
            //Debug.Log("4");
